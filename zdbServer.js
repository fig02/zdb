function Breakpoint(enabled, funcName, addr) {
    ...
    if (this.enabled) {
        ...
        console.log('created breakpoint at ' + addr.toString(16));
    } else {
        this.id = null;
    }
}
...
Breakpoint.prototype.setOvl = function(name, offset) {
    console.log('setting ovl');
    
    this.ovlName = name;
    this.ovlOffset = offset;

    if (name in ovlNameToBreakpoint) {
        ovlNameToBreakpoint[name].push(this);
    } else {
        ovlNameToBreakpoint[name] = new Array(this);
    }
};
...
// var testBreak = events.onexec(ADDR_ANY, function(pc) {
//     console.log('how many times, pc: ' + pc.hex());
// });
// events.remove(testBreak);
...
function processCommand(command) {
    ...
    if (split[0] == 'b') {
        if (split[2] == 'ovl') {
            ...
            var ovlName = split[3];
            var offset = parseInt(split[4]);
            var actorId = actorOverlays.indexOf(ovlName);
            if (actorId >= 0) {
                var actorTableBase = 0x801162A0;
                var actorTableEntry = actorTableBase + actorId * 0x20;
                // var numInstances = mem.u32[actorTableEntry + 0x1C];
                // if (numInstances == 0) {
                var actorFileBase = mem.u32[actorTableEntry + 0x10];
                if (actorFileBase == 0) {
                    // console.log('Error: all instances of actor overlay ' + ovlName + ' have been deallocated');
                    var new_breakpoint = new Breakpoint(null, false, funcName, null);
                    new_breakpoint.setOvl(ovlName, offset);
                } else {
                    // var actorFileStart = mem.u32[actorTableEntry + 0x10];
                    console.log('actor file start addr: ' + actorFileBase.toString(16));
                    var actorFileBreakAddr = actorFileBase + offset;
                    console.log('setting a breakpoint in actor file, at address ' + actorFileBreakAddr.toString(16));
                    var breakpointId = events.onexec(actorFileBreakAddr, function () {
                        debug.breakhere();
                    });

                    var newBreakpoint = new Breakpoint(breakpointId, true, funcName, actorFileBreakAddr);
                    newBreakpoint.setOvl(ovlName, offset);
                }

                if (ovlName in ovlNameToWatchpoint) {
                    ovlNameToWatchpoint[ovlName].push(watchpointId)
                } else {
                    console.log('setting watchpoint again');
                    var watchpointId = events.onwrite(actorTableEntry + 0x10, function(addr) {
                        // // TODO: add custom logic for determining what value will be written to addr given current opcode
                        // var newActorFileAddr = 0x1;
                        // if (newActorFileAddr != 0) {
                        //     // enable all breakpoints for actor
                        // } else {
                        //     // disable all breakpoints for actor
                        // }

                        // set breakpoint on any address, have this breakpoint read in the new value and then promptly delete itself
                        console.log('watchpoint fires');
                        pcOnWatch = gpr.pc;
                        var tempBreakpointId = events.onexec(ADDR_ANY, function(pc) {
                            // console.log('pc: ' + pc.hex());
                            if (pc === pcOnWatch) {
                                console.log('fired ADDR_ANY before pc incremented');
                            }
                            var newActorFileBase = mem.u32[addr];
                            if (newActorFileBase != 0) {
                                // enable all breakpoints at new addr for actor + offset
                                ovlNameToBreakpoint[ovlName].forEach(function(breakpoint) {
                                    console.log('base: ' + newActorFileBase);
                                    console.log('offset: ' + breakpoint.ovlOffset);
                                    breakpoint.enable(newActorFileBase + breakpoint.ovlOffset);
                                });
                            } else {
                                // disable all breakpoints for actor
                                ovlNameToBreakpoint[ovlName].forEach(function(breakpoint) {
                            ...
                            console.log('trying to remove id: ' + tempBreakpointId);
                            events.remove(tempBreakpointId);
                        });
                    });
                    ovlNameToWatchpoint[ovlName] = new Array(watchpointId);
                }

                return;
            }

            var particleId = particleOverlays.indexOf(ovlName);
            if (particleId >= 0) {
                var particleTableBase = 0x801159B0;
                var particleTableEntry = particleTableBase + particleId * 0x1C;
                var particleFileStart = mem.u32[particleTableEntry + 0x10];
                if (particleFileStart == 0) {
                    console.log('Error: particle file for overlay ' + ovlName + ' is not loaded into RAM');
                }
                var particleFileBreakAddr = particleFileStart + offset;
                console.log('setting a breakpoint in particle file, at address ' + particleFileBreakAddr.toString(16));
                events.onexec(particleFileBreakAddr, function () {
                    debug.breakhere();
                });
            ...
            var gamestateId = gamestateOverlays.indexOf(ovlName);
            if (gamestateId >= 0) {
                var gamestateTableBase = 0x8011F830;
                var gamestateTableEntry = gamestateTableBase + gamestateId * 0x30;
                var gamestateFileStart = mem.u32[gamestateTableEntry];
                if (gamestateFileStart == 0) {
                    console.log('Error: gamestate file for overlay ' + ovlName + ' is not loaded into RAM');
                }
                var gamestateFileBreakAddr = gamestateFileStart + offset;
                console.log('setting a breakpoint in gamestate file, at address ' + gamestateFileBreakAddr.toString(16));
                events.onexec(gamestateFileBreakAddr, function () {
                    debug.breakhere();
                });
            ...
            var pausePlayerId = pausePlayerOverlays.indexOf(ovlName);
            if (pausePlayerId >= 0) {
                var pausePlayerTableBase = 0x8011F830;
                var pausePlayerTableEntry = pausePlayerTableBase + pausePlayerId * 0x30;
                var pausePlayerFileStart = mem.u32[pausePlayerTableEntry];
                if (pausePlayerFileStart == 0) {
                    console.log('Error: Pause/Player file for overlay ' + ovlName + ' is not loaded into RAM');
                }
                var pausePlayerFileBreakAddr = pausePlayerFileStart + offset;
                console.log('setting a breakpoint in pause/player file, at address ' + pausePlayerFileBreakAddr.toString(16));
                events.onexec(pausePlayerFileBreakAddr, function () {
                    debug.breakhere();
                });
            ...
            var breakpointId = events.onexec(addr, function() {
                debug.breakhere();
            });

            Breakpoint(breakpointId, true, funcName, addr);
        }
    }
}
